...
export async function transcribeAndSummarize(input: InputAudio): Promise<TranscriptionResult> {
    ...
    // 1) Transcription with gpt-4o-mini-transcribe
    const transcript = await transcribe({ openaiKey, base64: input.base64, filename: input.filename });
...
async function transcribe({ openaiKey, base64, filename }: { openaiKey: string; base64: string; filename: string; }): Promise<string> {
...
${transcript}`;
    ...
    if (!res.ok) {
        const txt = await res.text();
        console.log('Summarization failed:', txt);
